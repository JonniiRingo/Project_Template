...
###############################################################################
# PRE-FLIGHT: offer to run the full mac dev–environment bootstrap
###############################################################################
if [[ -x "$(dirname "$0")/new_dev_onboard.sh" ]]; then
  read -rp "↪  Run full Mac dev-setup (Homebrew, Xcode, etc.)? [y/n] " yn
  if [[ ${yn,,} == y* ]]; then
    echo "☛ Launching new_dev_onboard.sh …"
    bash "$(dirname "$0")/new_dev_onboard.sh"
    echo "✔  System bootstrap finished.  Continuing with project spin-up …"
  else
    echo "ⓘ Skipping system bootstrap."
  fi
fi
